...
def model_SEResNet_aux(input_channels: int, input_size: int, aux_input_size: int, in_plane: int, num_block: int, num_aux=1, log_scale=False, activation="relu", backbone_act="relu", cuda_used=False, model_resaved=False, **kwargs) -> BuildingNet_aux:
    ...
    if model_resaved:
        if "saved_path_onnx" in kwargs.keys():
            saved_path_onnx = kwargs["saved_path_onnx"]
        else:
            saved_dir_torch = os.path.dirname(kwargs["trained_record"])
            saved_basename_torch = os.path.basename(kwargs["trained_record"])
            saved_path_onnx = os.path.join(saved_dir_torch, os.path.splitext(saved_basename_torch)[0] + "_ONNX" + ".onnx")
        ipt1_dummy = torch.randn(8, 6, 20, 20, device='cpu')
        ipt2_dummy = torch.randn(8, 1, 20, 20, device='cpu')
        torch.onnx.export(model, (ipt1_dummy, ipt2_dummy), saved_path_onnx)
...
if __name__ == "__main__":
    ...
    m = model_SEResNet_aux(in_plane=64, input_channels=6, input_size=20, aux_input_size=20, num_block=1, trained_record=pretrained_weight, model_resaved=False)
